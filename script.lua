...
_G.Config = {
    ...
    AimbotSmoothness = 0.25, -- 0.1 = Very Smooth, 1.0 = Hard Lock
...
local function CreateHealthUI(character, isPlayer)
    ...
    if _G.Config.HealthStyle == "Text Only" then
        ...
        local function UpdateText()
            ...
            txt.Text = (isPlayer and "👤 " or "👾 ") .. math.floor(math.max(0, hum.Health)) .. " / " .. math.floor(hum.MaxHealth)
        end
        UpdateText()
        hum.HealthChanged:Connect(UpdateText)
    elseif _G.Config.HealthStyle == "Classic Bar" then
        local bg = Instance.new("Frame")
        bg.Size = UDim2.new(1, 0, 0, 8); bg.Position = UDim2.new(0, 0, 0.5, -4); bg.BackgroundColor3 = Color3.fromRGB(20, 20, 20); bg.BorderSizePixel = 0; bg.Parent = bb
        local fill = Instance.new("Frame")
        fill.Size = UDim2.new(math.clamp(hum.Health / math.max(1, hum.MaxHealth), 0, 1), 0, 1, 0); fill.BackgroundColor3 = mainColor; fill.BorderSizePixel = 0; fill.Parent = bg
...
local function IsWhitelisted(name)
    for _, v in ipairs(_G.Config.Whitelist) do if v:lower() == name:lower() then return true end end
    return false
end

local function IsTargeted(name)
    if #_G.Config.TargetList == 0 then return true end
    for _, v in ipairs(_G.Config.TargetList) do if v:lower() == name:lower() then return true end end
    return false
end
...
RunService.RenderStepped:Connect(function()
    ...
    if isAimbotActive then
        ...
        if lockedTargetPart and lockedTargetPart.Parent then
            pcall(function()
                -- CAMERA LERPING (Smooth instead of hard-locking)
                ...
                -- LAG OPTIMIZATION: Only update hitbox properties if they haven't been applied yet
...
TitleLabel.Size = UDim2.new(1, 0, 0, 42); TitleLabel.Text = "⚔️ PVP COMBAT"; TitleLabel.Font = Enum.Font.GothamBold; TitleLabel.TextSize = 11; TitleLabel.BackgroundTransparency = 1; TitleLabel.Parent = Sidebar

local ContentArea = Instance.new("Frame")
ContentArea.Size = UDim2.new(1, -150, 1, -10); ContentArea.Position = UDim2.new(0, 145, 0, 5); ContentArea.BackgroundTransparency = 1; ContentArea.Parent = MainFrame

local ToggleBtn = Instance.new("TextButton")
ToggleBtn.Size = UDim2.new(0, 65, 0, 32); ToggleBtn.Position = UDim2.new(0.02, 0, 0.05, 0); ToggleBtn.Text = "PVP"; ToggleBtn.Font = Enum.Font.GothamBold; ToggleBtn.TextSize = 12; ToggleBtn.Parent = ScreenGui
...
local function CreateTab(tabName)
    ...
    local function UpdateTabStyle(theme)
        BtnCorner.CornerRadius = UDim.new(0, math.max(4, theme.CornerRadius - 4))
        if TabPage.Visible then TabBtn.TextColor3 = theme.Accent; TabBtn.BackgroundColor3 = theme.CardBg else TabBtn.TextColor3 = theme.SubText; TabBtn.BackgroundColor3 = theme.SidebarBg end
    end
...
AddDropdown(CombatTab, "Aimbot Smoothness", {"0.1", "0.25", "0.5", "0.75", "1.0"}, "AimbotSmoothness") -- 0.1 = Silky, 1.0 = Instant
