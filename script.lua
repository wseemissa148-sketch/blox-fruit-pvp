...
-- Apply initial theme
ApplyTheme()عدل السكربت وخليه الابموت الها ثلاث انواع قوي كثير ومتوسط وخفيف وحط في FAST ATTACK نفس الاشي ثلاث انواع وخليهم كلهم كبس يعني كل اشي اله كبسه خاصة وr3 = theme.Accent
    end
    table.insert(ThemeUpdateSignals, UpdateBindStyle)
...
-- Apply initial theme
ApplyTheme()عدل السكربت وخليه الابموت الها ثلاث انواع قوي كثير ومتوسط وخفيف وحط في FAST ATTACK نفس الاشي ثلاث انواع وخليهم كلهم كبس يعني كل اشي اله كبسه خاصة وبدون لاق
    Whitelist = {},
    TargetList = {}
}
...
-- ==========================================
-- 3. ADVANCED AIMBOT & FAST ATTACK ENGINE
-- ==========================================
    ...
    if isAimbotActive then
        ...
        if lockedTargetPart and lockedTargetPart.Parent then
            pcall(function()
                ...
                if rootPart then
                    local distance = (rootPart.Position - lockedTargetPart.Position).Magnitude
                    
                    if distance < 8 then
                        if originalSizes[lockedTargetPart] then
                            lockedTargetPart.Size = originalSizes[lockedTargetPart]
                        end
                        lockedTargetPart.Transparency = 0
                        lockedTargetPart.CanCollide = true
                    else
                        if not originalSizes[lockedTargetPart] then
                            originalSizes[lockedTargetPart] = lockedTargetPart.Size
                        end
...
task.spawn(function()
    while true do
        if _G.Config.FastAttackEnabled then
            pcall(function()
                local mode = _G.Config.FastAttackMode
...
-- زر الإيقونة القابل للسحب (Draggable Toggle Button)
...
local function AddDropdown(parentPage, title, optionsList, configVar)
    ...
    table.insert(ThemeUpdateSignals, Upd-- ==========================================================
...
-- Apply initial theme
ApplyTheme()gnment = Enum.TextXAlignment.Left
        title.Parent = card
    ...
    if isAimbotActive then
        ...
        if lockedTargetPart and lockedTargetPart.Parent then
            pcall(function()
                ...
                -- التحقق من المسافة بين اللاعب والخصم
                local localChar = LocalPlayer.Character
                local rootPart = localChar and localChar:FindFirstChild("HumanoidRootPart")
                ...
                if rootPart then
                    ...
                    -- إذا كان القرب أقل من 8 ستودز، ارجع الحجم الأصلي لمنع التخبط، وإلا طبق الحجم المعدل
                    if distance < 8 then
                        if originalSizes[lockedTargetPart] then
                            lockedTargetPart.Size = originalSizes[lockedTargetPart]
                        end
...
UserInputService.InputBegan:Connect(function(input, processed)
    ...
    pcall(function()
        ...
        elseif input.KeyCode == Enum.KeyCode[_G.Config.FastAttackKey] then
            _G.Config.FastAttackEnabled = not _G.Config.FastAttackEnabled
        end
    end)
end)
...
AddToggle(CombatTab, "Mob Camera Aimbot", "MobAimbot")
AddKeybindPicker(CombatTab, "Mob Aimbot Key", "MobAimbotKey")
AddDropdown(CombatTab, "Aimbot Strength", {"قوي جداً", "متوسط", "خفيف"}, "AimbotMode")

AddToggle(CombatTab, "Fast Attack (الضرب السريع)", "FastAttackEnabled")
AddKeybindPicker(CombatTab, "Fast Attack Key", "FastAttackKey")
AddDropdown(CombatTab, "Fast Attack Speed", {"قوي جداً", "متوسط", "خفيف"}, "FastAttackMode")
...
-- Settings / Themes Tab
AddDropdown(SettingsTab, "UI Theme Model", {"Cyber Dark", "Midnight Purple", "Emerald Neon", "Crimson Red", "Minimal Light"}, "CurrentTheme")

ApplyTheme()    MobHealthColor = {R = 255, G = 50, B = 50},
...
-- Aimbot Loop with 3 Types (قوي جداً, متوسط, خفيف)
RunService.RenderStepped:Connect(function()
    local isAimbotActive = _G.Config.PlayerAimbot or _G.Config.MobAimbot
    ...
    if isAimbotActive then
        ...
        if lockedTargetPart and lockedTargetPart.Parent then
            pcall(function()
                ...
                if mode == "قوي جداً" then
                    -- قفل تام وفوري بدون تأخير
                    Camera.CFrame = targetCFrame
                elseif mode == "متوسط" then
                    -- سلاسة متوسطة سريعة ومريحة
                    Camera.CFrame = Camera.CFrame:Lerp(targetCFrame, 0.35)
                elseif mode == "خفيف" then
                    -- تتبع خفيف وناعم جداً (Legit)
                    Camera.CFrame = Camera.CFrame:Lerp(targetCFrame, 0.12)
                end

                -- تعديل الهيت بوكس حسب الرغبة وبدون لاق
                lockedTargetPart.Size = Vector3.new(_G.Config.HitboxSize, _G.Config.HitboxSize, _G.Config.HitboxSize)
                lockedTargetPart.Transparency = 0.7
                lockedTargetPart.CanCollide = false
            end)
...
-- Fast Attack Engine with 3 Types (قوي جداً, متوسط, خفيف)
task.spawn(function()
    while true do
        if _G.Config.FastAttackEnabled then
            pcall(function()
                ...
                if tool then
                    -- محاكاة الهجوم السريع عبر تفعيل الـ Tool بشكل متكرر حسب السرعة المطلوبة
                    tool:Activate()
                end
